    ...
    for(int y=0; y<height; y++){
        for(int x=0; x<width; x++){
            out.at<uchar>(y,x) = (int)((float)img.at<Vec3b>(y,x)[0] * 0.0722 + \
                                        (float)img.at<Vec3b>(y,x)[1] *0.7152 + \
                                        (float)img.at<Vec3b>(y,x)[2] * 0.2126);
        }
    }
    return out;
}
...
Mat soble_filter(Mat img,int kernel_size,bool horizontal) {
    int height = img.rows;
    int width = img.cols;
    int channel = img.channels();
...
Mat non_maximum_suppression(Mat angle,Mat edge){
